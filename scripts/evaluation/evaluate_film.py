"""
Evaluates a trained FiLMClassifier (from train_film_normalize.py ONLY) on the TEST
split, broken out per variant. Reports accuracy + AUC for each of the 16 variants
(clean + 15 robustness transforms) individually, directly comparable to
evaluate_base.py's and evaluate_concat.py's tables -- same test images, same metrics,
...
train_film_normalize.py.

Run this AFTER train_film_normalize.py has produced a checkpoint. Touches test.npz,
which nothing else in the pipeline reads -- meant to be run once, at the end.

Usage:
    python scripts/evaluate_film.py
    python scripts/evaluate_film.py --checkpoint checkpoints/model_film_normalized.pt
"""
import argparse
import sys
...
def load_by_variant(embeddings_path: Path, stats_path: Path, mean: np.ndarray, std: np.ndarray) -> dict:
    ...
    generator_family_array, img_ids_array). Unlike train_film_normalize.py's
    ...
    Arrays are pulled out of both NpzFiles ONCE, up front -- indexing data[key][i] in a
    loop re-reads the whole array from disk on every access (a correctness/memory bug,
    not just a style choice -- this is what OOM-crashed the very first DCPT run).
...
def main() -> None:
    ...
    ap.add_argument("--checkpoint", default="checkpoints/model_film_normalized.pt")
    ...
    ap.add_argument("--errors-dir", default="results/errors",
                     help="Where film_fp.csv / film_fn.csv (every variant, threshold 0.5) get written.")
    args = ap.parse_args()
    ...
    write_error_csv(errors_dir / "film_fp.csv", fp_all)
    write_error_csv(errors_dir / "film_fn.csv", fn_all)
    print(f"\nWrote {len(fp_all)} false positive(s) to {errors_dir / 'film_fp.csv'}")
    print(f"Wrote {len(fn_all)} false negative(s) to {errors_dir / 'film_fn.csv'}")
