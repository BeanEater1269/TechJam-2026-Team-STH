...
def load_by_variant(npz_path: Path) -> dict:
    ...
    source_dataset_array, generator_family_array), across ALL images -- unlike
    train_base.py's group_by_image() (which groups by img_id, for DCPT pairing), this
    groups by variant, since the robustness table needs "every jpeg_q30 row" as one
    batch, not paired with anything. source_dataset/generator_family are carried
    ...
    return {v: (np.stack(e), np.array(l), np.array(s), np.array(g))
            for v, (e, l, s, g) in by_variant.items()}
...
def main() -> None:
    ...
    for v in variant_order:
        embeddings, labels, source_ds, gen_fam = by_variant[v]
