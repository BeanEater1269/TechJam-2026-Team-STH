...
training/train_film_normalize.py, training/train_film_normalize_drift.py, then
evaluation/evaluate_base.py, evaluation/evaluate_concat.py, evaluation/evaluate_film.py,
evaluation/evaluate_film_drift.py in sequence, and saves each run's results into a new
results/ directory as JSON -- tagged with which model (base/concat/film/film_drift),
which signals were used, and which CLIP backbone (default ViT-B/32, backbone_dim=512).
...
the 4 B-signals' {split}_signals.npz aren't either.

Nothing is parsed by hand-inspecting output -- each script's stdout is machine-parsed
against the exact print formats those scripts use (same formats across all 4 training
scripts, and across all 4 eval scripts), so results are pulled as real numbers, not
copy-pasted.

WARNING: this retrains all four models from scratch, overwriting whatever's currently
