...
training/train_film_normalize.py, training/train_film_normalize_drift.py,
training/train_film_drift_liqe.py, then evaluation/evaluate_base.py,
evaluation/evaluate_concat.py, evaluation/evaluate_film.py, evaluation/evaluate_film_drift.py,
evaluation/evaluate_film_drift_liqe.py in sequence, and saves each run's results into a
new results/ directory as JSON -- tagged with which model
(base/concat/film/film_drift/film_drift_liqe), which signals were used, and which CLIP
backbone (default ViT-B/32, backbone_dim=512).

film_drift needs data/cache/stats/{train,val,test}_drift.npz to already exist (run
scripts/features/clip_drift.py first) -- it is NOT generated by this script, same as
the 4 B-signals' {split}_signals.npz aren't either. film_drift_liqe additionally needs
...
JOBS = [
    ...
    ("film_drift_train", "training/train_film_normalize_drift.py", "train", "film_drift", DRIFT_SIGNAL_COLUMNS, True),
    ("film_drift_liqe_train", "training/train_film_drift_liqe.py", "train", "film_drift_liqe", DRIFT_LIQE_SIGNAL_COLUMNS, True),
    ...
    ("film_drift_eval", "evaluation/evaluate_film_drift.py", "eval", "film_drift", DRIFT_SIGNAL_COLUMNS, True),
    ("film_drift_liqe_eval", "evaluation/evaluate_film_drift_liqe.py", "eval", "film_drift_liqe", DRIFT_LIQE_SIGNAL_COLUMNS, True),
]
