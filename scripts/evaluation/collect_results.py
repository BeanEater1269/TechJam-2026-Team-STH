...
training/train_concat_normalize_drift.py, training/train_concat_drift_liqe.py,
training/train_film_normalize.py, then evaluation/evaluate_base.py,
evaluation/evaluate_concat.py, evaluation/evaluate_concat_drift.py,
evaluation/evaluate_concat_drift_liqe.py, evaluation/evaluate_film.py in sequence, and
saves each run's results into a new results/ directory as JSON -- tagged with which
model (base/concat/concat_drift/concat_drift_liqe/film), which signals were used, which
CLIP backbone (default ViT-B/32, backbone_dim=512), and which device (cuda/cpu) that job
actually ran on.

Final model settled on: concat_drift. film_drift/film_drift_liqe (and their train/eval
scripts) were dropped from this pipeline once that decision was made -- plain `film` is
kept only as a baseline comparison point, not as a candidate. concat_drift_liqe is kept
too, as the "we tried adding LIQE and it made concat_drift worse" comparison point for
the trade-offs writeup.

concat_drift needs data/cache/stats/{train,val,test}_drift.npz to already exist (run
scripts/features/clip_drift.py first) -- NOT generated by this script, same as the 4
B-signals' {split}_signals.npz aren't either. concat_drift_liqe additionally needs
data/cache/stats/{train,val,test}_liqe.npz (run scripts/features/liqe.py first).

Nothing is parsed by hand-inspecting output -- each script's stdout is machine-parsed
against the exact print formats those scripts use (same formats across all 5 training
scripts, and across all 5 eval scripts), so results are pulled as real numbers, not
copy-pasted.

WARNING: this retrains all five models from scratch, overwriting whatever's currently
...
since GPU op nondeterminism isn't separately forced here (this is also why `device` is
now recorded per job -- a run that silently fell back to CPU produces a genuinely
different trained model, not just a slower one, so it needs to be visible in the
output, not just inferred from wall-clock time).
...
DRIFT_SIGNAL_COLUMNS = SIGNAL_COLUMNS + ["clip_drift"]
DRIFT_LIQE_SIGNAL_COLUMNS = DRIFT_SIGNAL_COLUMNS + ["liqe_score"]
...
JOBS = [
    ...
    ("concat_drift_train", "training/train_concat_normalize_drift.py", "train", "concat_drift", DRIFT_SIGNAL_COLUMNS, True),
    ("concat_drift_liqe_train", "training/train_concat_drift_liqe.py", "train", "concat_drift_liqe", DRIFT_LIQE_SIGNAL_COLUMNS, True),
    ...
    ("concat_drift_eval", "evaluation/evaluate_concat_drift.py", "eval", "concat_drift", DRIFT_SIGNAL_COLUMNS, True),
    ("concat_drift_liqe_eval", "evaluation/evaluate_concat_drift_liqe.py", "eval", "concat_drift_liqe", DRIFT_LIQE_SIGNAL_COLUMNS, True),
    ("film_eval", "evaluation/evaluate_film.py", "eval", "film", SIGNAL_COLUMNS, True),
]

# Every job script's very first print is "device: cuda" or "device: cpu, signal_dim:
# ..." -- captured separately from parse_train_output/parse_eval_output (which only
# look at metrics lines) since it's the same one-line fact regardless of train/eval.
DEVICE_RE = re.compile(r"^device:\s*([a-zA-Z0-9_]+)", re.MULTILINE)
...
def parse_device(output: str) -> str | None:
    """Pulls the device (cuda/cpu) each job script prints on its own first line.
    None (not a crash) if the format ever changes -- this is provenance metadata, not
    something that should block a whole run over a missed print."""
    m = DEVICE_RE.search(output)
    if m is None:
        print("  WARNING: no 'device: ...' line matched -- can't record which device this job ran on")
        return None
    return m.group(1)
...
def main() -> None:
    ...
    for run_name, script, kind, model_label, signals, signals_normalized in JOBS:
        ...
        device_used = parse_device(output)
    ...
    devices_used = {row["device"] for row in summary_rows}
    if "cpu" in devices_used:
        cpu_runs = [row["run"] for row in summary_rows if row["device"] == "cpu"]
        print(f"\n  WARNING: {len(cpu_runs)} job(s) ran on CPU, not CUDA: {', '.join(cpu_runs)} -- "
              f"if that wasn't intended, check which `python` your shell resolved to "
              f"(bare `python` vs .venv\\Scripts\\python.exe) and re-run.")
