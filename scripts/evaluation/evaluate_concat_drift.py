"""
Evaluates a trained ConcatClassifier (from train_concat_normalize_drift.py ONLY -- the
4-signal train_concat_normalize.py/model_concat_normalized.pt path is a separate,
untouched baseline) on the TEST split, broken out per variant. Same table shape as
evaluate_concat.py -- same test images, same metrics -- except this model takes 5
signals (the 4 B-signals + clip_drift) instead of 4.
...
as train_concat_normalize_drift.py's load_merged_split(), just grouped by variant here
instead of by img_id (the robustness table needs "every jpeg_q30 row" as one batch, not
paired for DCPT).

Run this AFTER train_concat_normalize_drift.py has produced a checkpoint. Touches
test.npz, which nothing else in the pipeline reads -- meant to be run once, at the end.

Usage:
    python scripts/evaluation/evaluate_concat_drift.py
    python scripts/evaluation/evaluate_concat_drift.py --checkpoint checkpoints/model_concat_drift_normalized.pt
"""
import argparse
import sys
...
def load_by_variant(
    ...
    train_concat_normalize_drift.py's load_merged_split() (grouped by img_id, for DCPT
    ...
    train_concat_normalize_drift.py's load_merged_split() for why indexing data[key][i]
    in a loop is a correctness/memory bug, not just a style choice.
...
def main() -> None:
    ...
    ap.add_argument("--checkpoint", default="checkpoints/model_concat_drift_normalized.pt")
    ...
    ap.add_argument("--errors-dir", default="results/errors",
                     help="Where concat_drift_fp.csv / concat_drift_fn.csv (every variant, threshold 0.5) get written.")
    args = ap.parse_args()
    ...
    by_variant = load_by_variant(
        emb_root / "test.npz", stats_root / "test_signals.npz", stats_root / "test_drift.npz", mean, std)
    ...
    write_error_csv(errors_dir / "concat_drift_fp.csv", fp_all)
    write_error_csv(errors_dir / "concat_drift_fn.csv", fn_all)
    print(f"\nWrote {len(fp_all)} false positive(s) to {errors_dir / 'concat_drift_fp.csv'}")
    print(f"Wrote {len(fn_all)} false negative(s) to {errors_dir / 'concat_drift_fn.csv'}")
