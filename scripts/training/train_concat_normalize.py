"""
Same as train_concat.py (DCPT + ConcatClassifier), except the 4 raw B-signals get
z-score normalized before they're concatenated with the CLIP embedding.

signals.py saves raw, unscaled pixel measurements (Laplacian variance, DCT band
energies, noise variance) -- these can sit on very different numeric scales from each
other and from CLIP's roughly unit-scale embedding values. Concatenating them straight
into a Linear layer with no normalization risks one large-magnitude signal dominating
the first layer's gradients regardless of whether it's actually informative. This is
suspected to be why the un-normalized model_concat run scored BELOW the zero-signal
base_classifier baseline on every metric.

Normalization stats (mean, std) come from normalize.py, computed ONCE from the TRAIN
split's raw signal file, then applied unchanged to train, val, and (in
evaluate_concat.py) test -- never recomputed per split.

Usage:
    python scripts/train_concat_normalize.py
    python scripts/train_concat_normalize.py --epochs 10 --backbone-dim 768
"""
import argparse
import random
import sys
...
def main() -> None:
    ...
    ap.add_argument("--out", default="checkpoints/model_concat_normalized.pt")
