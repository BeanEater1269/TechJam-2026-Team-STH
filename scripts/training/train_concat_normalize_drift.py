"""
Same as train_concat_normalize.py (DCPT + ConcatClassifier, 4 z-scored B-signals), plus a
...
Usage:
    python scripts/training/train_concat_normalize_drift.py
    python scripts/training/train_concat_normalize_drift.py --epochs 10 --backbone-dim 768
"""
import argparse
import random
import sys
...
def load_merged_split(
    ...
    -- same reasoning as train_concat_normalize.py's load_merged_split().
...
def main() -> None:
    ...
    ap.add_argument("--out", default="checkpoints/model_concat_drift_normalized.pt")
